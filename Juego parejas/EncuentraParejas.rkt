...
;Dibujo las ventanas
(define ventana (open-viewport "Memoria" 630 640))
(define fondo (open-pixmap "Memoria" 630 640))
...
;Creo los cuadros de la puntuacion, aciertos y errores
...
;Sonido al clickear un contenedor

(define (voltear) (play-sound "Sounds/mouth07.wav" #t))

;Guardo las listas de imagenes y posiciones
;Cada imagen estara repetida dos veces
...
;Aqui creo una lista que cambie el orden de las imagenes aleatoriamente (que no esten siempre en el mismo sitio)
...
;Aqui defino una funcion que cuente los segundos desde 0
(define (contador_segundos n)
  (- (current-seconds) tiempo_inicial))

;Aqui defino la funcion para saber si el click se hizo en la dimension de alguna imagen
...
;Esta funcion da la posicion del contenedor donde se hizo click   
...
;Esta funcion realiza el cambio de estado de una imagen en un vector
(define (cambiar_estado_imagen k imagen)
  (begin (voltear)(vector-set! estado_imagen k imagen) estado_imagen))

;Funcion que determina si n=1
(define (igual? n)
  (if (equal? n 1) #t #f))
...
(define (juego n s minutos click)
 (cond
   [(equal? (vector-ref aciertos_errores 0) 8)  
    (begin
      (define ventana2 (open-viewport "ganador" 630 640))
                                                 (define fondoganador (open-pixmap "ganador" 630 640))
                                        
                                              
                                                 (((draw-pixmap-posn "Imagenes/ganaste.gif" 'gif/mask) ventana2)(make-posn 0 0 ))
